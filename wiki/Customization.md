...
- **GTK3/GTK4** — `~/.config/gtk-3.0/settings.ini` sets `gtk-application-prefer-dark-theme=1` with `Adwaita` theme and `Papirus-Dark` icons
- **Qt5** — `~/.config/qt5ct/qt5ct.conf` uses `Breeze-Dark` style
- **Qt6** — Auto-generated by wallust at `~/.config/qt6ct/qt6ct.conf` with `Breeze-Dark` style and wallpaper-matched custom palette
- **Environment** — `~/.config/environment.d/qt.conf` sets `QT_STYLE_OVERRIDE=breeze-dark` and `QT_QPA_PLATFORMTHEME=qt6ct`

To switch to light theme, edit the above files and change `breeze-dark` → `breeze`, `Papirus-Dark` → `Papirus`, and set `prefer-dark-theme=0`.
